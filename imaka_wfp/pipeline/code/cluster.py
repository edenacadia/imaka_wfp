...
class Cluster(object):
    # takes in very specific 
    
    def __init__ (self, clstr_type):
        self.clstr_method = detect_cluster_meanshift
        if clstr_type == "meanshift":
            self.clstr_method = detect_cluster_meanshift
        elif clstr_type == "db_scan":
            self.clstr_method = detect_cluster_dbscan
        elif clstr_type == "affprop":
            self.clstr_method = detect_cluster_affprop
            
    def run(data, vspd, vedgex, vedgey):
        return self.clstr_method(data, vspd, vedgex, vedgey)
...
#Gobal
clstr_list = ["meanshift", "db_scan", "affprop"]
dispatcher={'meanshift':detect_cluster_meanshift,
           'db_scan':detect_cluster_dbscan,
           'affprop':detect_cluster_affprop}